...
log "===== 修复完成 ====="
log "验证步骤:"
log "  1. pm2 logs perf-app --lines 30"
log "  2. 浏览器打开 https://${SERVER_NAME}/ 登录审核端"
log "  3. 打开申诉附件；Network 中 viewUrl 应为 /api/attachments/.../view?proxy=1（proxy 模式）"
log "  4. 若仍裂图，确认对象是否在桶内（本迁移默认不迁 MinIO 对象）"
