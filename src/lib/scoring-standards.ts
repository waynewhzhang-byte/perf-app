...
/** 二级维度：Excel「评价内容」+ 系统 dimensionCode（由 SCORING_STANDARDS 派生） */
...
/**
 * 评分规则管理页用的维度列表（无导入字段清单）。
 * 字段映射见 admin/import/_shared/field-specs.ts。
 */
export interface ScoringDimensionListItem {
  code: string;
  name: string;
  category: string;
  sectionCode: PerformanceSectionCode;
...
export const DIMENSION_DEFS: ScoringDimensionListItem[] = PERFORMANCE_SUB_DIMENSIONS.map((d) => ({
  code: d.code,
  name: d.title,
...
/**
 * 外部台账维度快捷引用：从 `SUB_DIMENSION_BY_CODE` 取值，避免各调用方重复查找。
 */
